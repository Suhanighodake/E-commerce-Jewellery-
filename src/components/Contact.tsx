...
export default function Contact() {
  return (
    <section id="contact" className="relative py-10 md:py-14 bg-slate-50 overflow-hidden">
      ...
      <div className="container-luxury relative z-10">
        <ScrollReveal className="text-center mb-14">
          <span className="text-brand-teal text-sm uppercase tracking-[0.25em] font-medium">Get in Touch</span>
          <h2 className="font-[Cormorant_Garamond] text-5xl md:text-6xl lg:text-7xl font-light mt-3 mb-4 text-slate-800">
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 max-w-5xl mx-auto">
          <StaggerContainer staggerDelay={0.1}>
                ...
                return (
                  <StaggerItem key={info.title}>
                    <motion.div whileHover={{ y: -3 }}
                      ...
                      <h3 className="font-[Cormorant_Garamond] text-lg font-semibold text-slate-800 mb-2">{info.title}</h3>
                      {info.details.map((d, i) => <p key={i} className="text-sm text-slate-400 leading-[1.7]">{d}</p>)}
                    </motion.div>
                  </StaggerItem>
                );
              })}
            ...
            <div className="mt-5 flex items-center gap-2">
              <span className="text-sm text-slate-400 uppercase tracking-wider">Follow:</span>
              {[Globe, ExternalLink].map((Icon, i) => (
                <motion.a key={i} href="#" whileHover={{ scale: 1.08 }}
                  className="w-9 h-9 rounded-full bg-white flex items-center justify-center text-slate-400 hover:text-brand-teal transition-all border border-slate-100 shadow-sm">
          ...
          <ScrollReveal direction="right">
            <form className="bg-white/80 backdrop-blur-sm rounded-sm p-6 sm:p-8 border border-slate-100 shadow-card">
              <h3 className="font-[Cormorant_Garamond] text-2xl font-semibold text-slate-800 mb-6">Send a Message</h3>
              <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm text-brand-teal uppercase tracking-[0.15em] font-medium mb-2">First Name</label>
                    <input type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 text-slate-700 text-base focus:outline-none focus:border-brand-teal transition-all placeholder:text-slate-400 placeholder:text-base rounded-sm"
                      placeholder="Your name" />
                  </div>
                  <div>
                    <label className="block text-sm text-brand-teal uppercase tracking-[0.15em] font-medium mb-2">Last Name</label>
                    <input type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 text-slate-700 text-base focus:outline-none focus:border-brand-teal transition-all placeholder:text-slate-400 placeholder:text-base rounded-sm"
                      placeholder="Your surname" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm text-brand-teal uppercase tracking-[0.15em] font-medium mb-2">Email</label>
                  <input type="email" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 text-slate-700 text-base focus:outline-none focus:border-brand-teal transition-all placeholder:text-slate-400 placeholder:text-base rounded-sm"
                    placeholder="you@email.com" />
                </div>
                <div>
                  <label className="block text-sm text-brand-teal uppercase tracking-[0.15em] font-medium mb-2">Message</label>
                  <textarea rows={4} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 text-slate-700 text-base focus:outline-none focus:border-brand-teal transition-all resize-none placeholder:text-slate-400 placeholder:text-base rounded-sm"
